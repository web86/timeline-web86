...
function web86_timeline_init()
{

    /**
     * Metabox form template
     */
    function web86_timeline_meta_box_callback($post)
    {
        ...
        <div id="web86_timeline">
            ...
            <div class="wfieldset">
                ...
                foreach ($value[0] as $key => $group) {
                    ...
                    <label class="mainlabel"><span class="count">' . (esc_html($key) + 1) . ')</span> Section name:</label>';
                    ...
                    foreach ($group as $fieldkey => $input_value) {
                        if (!is_array($input_value)) {
                            ...
                            echo '<input type="' . esc_html($typeinpit) . '" name="web86_field_name[0][' . (esc_html($key) + 1) . '][' . esc_html($fieldkey) . ']" value="' . esc_html($input_value) . '" class="' . esc_html($myclass) . '" placeholder="' . esc_html($placeholder) . '">';
                        }
                    }
                        ...
                        if (is_array($input_value)) {
                            foreach ($input_value as $subfield_key => $subfield_value) {
                                ...
                                foreach ($subfield_value as $subfield_value_key => $subfield_value_val) {
                                    echo '<textarea rows="4" name="web86_field_name[0][' . (esc_html($key) + 1) . '][' . esc_html($fieldkey) . '][' . esc_html($subfield_key) . '][' . esc_html($subfield_value_key) . ']" style="width:100%">' . esc_html($subfield_value_val) . '</textarea>';
                                }
     ...
     */
    function web86_timeline_add_meta_box()
    {
        $allowed_page_ids = get_option('web86_timeline_pages', array());
        $allowed_post_ids = get_option('web86_timeline_posts', array());
        ...
        if (in_array(get_the_ID(), $allowed_post_ids) || in_array(get_the_ID(), $allowed_page_ids)) {
            add_meta_box(
                ...
                'Web86 Timeline',
                // Meta block header
                'web86_timeline_meta_box_callback',
    ...
    add_action('add_meta_boxes', 'web86_timeline_add_meta_box'); // Adding a meta block

    function web86_timeline_save_meta_box_data($post_id)
    {
        ...
        $web86_field_name = array(); // Создаем пустой массив

        // if ( isset( $_POST['my_nonce'] ) && wp_verify_nonce( $_POST['my_nonce'], 'my_action' ) && current_user_can( 'manage_options' ) ) {

        //     $web86_field_name[0] = array_map( 'sanitize_group', $_POST['web86_field_name'][0] );
        //     $web86_field_name[1] = wp_kses( $_POST['web86_field_name'][1], $allowed_tags );

        //     // Другой код для обработки данных формы

        // } else {
        //     // Обработка ошибки, если nonce не прошел проверку или у пользователя нет прав доступа
        // }

        // // Функция для санитизации каждой группы данных
        // function sanitize_group( $group ) {
        //     $clean_group = array();
    ...
    add_action('save_post', 'web86_timeline_save_meta_box_data');

    // register the fields for the settings page in the menu
    register_setting('web86_timeline_plugin_settings', 'web86_timeline_pages');
    register_setting('web86_timeline_plugin_settings', 'web86_timeline_posts');
}
add_action('admin_init', 'web86_timeline_init');


// Include the script
function web86_timeline_plugin_enqueue_scripts($hook)
{
    if ('post.php' == $hook || 'post-new.php' == $hook || 'page.php' == $hook || 'page-new.php' == $hook) {
        // adding our own scripts and styles
        wp_enqueue_script(
            'web86-plugin-script',
            plugins_url('assets/js/web86-timeline-admin.js', __FILE__),
            array('jquery'),
            '1.0.0',
            true
        );
        wp_enqueue_style('web86-plugin-style', plugins_url('assets/css/web86-timeline-admin.css', __FILE__));

    }
}
add_action('admin_enqueue_scripts', 'web86_timeline_plugin_enqueue_scripts');


// Create a shortcode
function web86_timeline_plugin_register_shortcodes()
{

    // inqlude a file with a shortcode
    require_once(plugin_dir_path(__FILE__) . 'shortcode.php');

    add_shortcode('web86_timeline', 'web86_timeline_shortcode_function');
}
add_action('init', 'web86_timeline_plugin_register_shortcodes');
...
function web86_timeline_settings()
{
    ...
    add_options_page(
        ...
        'manage_options',
        // access level
        'web86_timeline_plugin_settings',
        // unique page ID
        'web86_timeline_settings_page' // the function of displaying the settings page
    );
}
...
{
    delete_option('web86_timeline_pages');
    delete_option('web86_timeline_posts');
}
